...
const ShineButton = React.forwardRef<HTMLButtonElement, ShineButtonProps>(
  ({ children, ...props }, ref) => {
    return (
      ...
      >
        {children}
        <div className="absolute inset-0 flex h-full w-full justify-center [transform:skew(-13deg)_translateX(-100%)] group-enabled:group-hover/button:duration-1000 group-enabled:group-hover/button:[transform:skew(-13deg)_translateX(100%)]">
